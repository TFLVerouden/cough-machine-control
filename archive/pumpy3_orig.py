...
class Pump:
    ...
    def setdiameter(self, diameter):
        ...
        if self.state == 'idle':
            ...
            if (last_line[2] == ':' or last_line[2] == '<' or last_line[
                2] == '>'):
                ...
                raise PumpError(
                    '%s: unknown response to setdiameter' % self.name)
        else:
    ...
    def setwithdrawrate(self, flowrate, unit):
        ...
        if self.state == 'idle':
            ...
            if (resp[2] == ':' or resp[2] == '<' or resp[2] == '>'):
                ...
                returned_flowrate = convert_units(float(returned_flowrate),
                                                  returned_unit,
                                                  convert_str_units(unit))

                if returned_flowrate != flowrate:
                    logging.error('%s: set flowrate (%s %s) does not match'
                                  'flowrate returned by pump (%s %s)',
                                  self.name, flowrate, unit, returned_flowrate,
                                  unit)
        ...
        if self.state == 'idle':
            ...
            if True:
                ...
                if 'Target volume not set' in resp:
                    raise PumpError(
                        '%s: Target volume (%s %s) could not be set' %
                ...
                returned_targetvolume = convert_units(
                    float(returned_targetvolume), returned_unit + "/min",
    ...
    def setsyringevolume(self, vol, unit):
        ...
        if self.state == 'idle':
            ...
            if (resp[-1] == ':' or resp[-1] == '<' or resp[-1] == '>'):
                ...
                returned_volume = convert_units(float(returned_volume),
                                                returned_unit + "/min",
                                                convert_str_units(
                                                    unit + "/min"))
                ...
                if returned_volume != vol:
                    logging.error(
                        '%s: set syringe volume (%s %s) does not match'
