...
def sort_branches(branches):
    """
    This function just sorts the branches on length in reverse order.
    """
...
def branch_fitter(branch,image,dist,window=5, line_threshold=0.85,step_filament=1,target_y = 0.82, tolerance = 0.02,factor_outlier=2):
    ...
    line_threshold:  How light is the outside threshold basically where we stop adding points to the potential line
    step_filament: Whether to skip steps during tracking or not, not skipping=1
    target_y: where is the threshold where we say we are out of the filament
    factor_outlier:  If filaments width are this factor times standard deviation away from the mean they get scrapped, standard is 2: 95% CFI
...
def creating_pickles(folder,skip_first_files=800):
    numpy_array= []
    #match = re.search(r'\\([^\\]+?)', folder)
    match = re.search(r'\\(PEO[^\\]+)', folder)
    if match:
        result = match.group(1)
        savepath= r"D:\Experiments\Processed_Data\RemotePC\\Processed_arrays\\" + result +"fullframe.pkl"
        ...
        if True:
            ...
            filament_val =filament_file(folder,selected_image=-1,skip_first_files=skip_first_files,window_size=600)
...
base_path = r"D:\Experiments\sideview_coughs\29_07_25"
