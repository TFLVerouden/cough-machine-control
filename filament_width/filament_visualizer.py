...
def branch_fitter(branch,image,dist,window=5, line_threshold=0.85,step_filament=1,target_y = 0.82, tolerance = 0.02,factor_outlier=2):
    """
...
def filament_file(
    ...
    for i,file in enumerate(files):
        ...
        if i== selected_image or selected_image == -1:
            ...
            if i%skip_images==0 and i>skip_first_files:
                ...
                # ax[1].imshow(thresh)
                # plt.show()
                filament_visual = cv.cvtColor(thresh, cv.COLOR_GRAY2BGR)
                if len(contours)>0:
                    ...
                    for j,contour in enumerate(contours):
                        ...
                        if mask_bubbles[j]:
                            ...
                            if tag == "Filament":
                                ...
                                cv.drawContours(filament_visual,[contour],-1,(255,0,0),1)
                                cv.fillPoly(contour_mask, [contour], 255)
              
                    filament_visualizations.append(filament_visual) 
    ...
    return all_length_time,filament_visualizations
...
def branch_combiner(skeleton,min_length=30):
...
def scale_ticks_x(scale):
    return lambda value, pos: f"{(55+value) / scale:.1f}"
def scale_ticks_y(scale):
    return lambda value, pos: f"{value / scale:.1f}"
...
def creating_pickles(folder,skip_first_files=250):
    ...
    if match:
        ...
        savepath= r"D:\Experiments\Processed_Data\RemotePC\\Processed_arrays\\" + result +"test.pkl"
        save_video = r"D:\Experiments\Processed_Data\RemotePC\\Processed_arrays\\" + result +"red.mp4"
        ...
        if True:
            ...
            filament_val,frames =filament_file(folder,selected_image=-1,skip_first_files=skip_first_files,window_size=600)
            numpy_array.append(filament_val)
            fig, ax = plt.subplots()
            im = ax.imshow(frames[0], cmap='gray', animated=True)
            ax.set_xlabel("X (mm)")
            ax.set_ylabel("Y (mm)")
            ax.xaxis.set_major_formatter(FuncFormatter(scale_ticks_x(15.67)))
            ax.yaxis.set_major_formatter(FuncFormatter(scale_ticks_y(15.67)))
            title =ax.set_title(f'Time: {round(skip_first_files/20000*1000,1)} ms')
            plt.tight_layout()
            def update(frame_idx):
                im.set_array(frames[frame_idx])
                title.set_text(f'Time: {round((skip_first_files+frame_idx)/20000*1000,1)} ms')
                return [im, title] 
...
base_path = r"D:\Experiments\Preliminary_tests\3103_FirstSpraytec"
...
folders = [
    ...
    if os.path.isdir(os.path.join(base_path, name)) and "0dot25" in name and "3rd" in name
]
print(folders)
len_folder = len(folders)
for l,folder in enumerate(folders):
    print(f"At {l}/{len_folder} experiments in this folder")
